      program adh_util_create_bed_th_th 
      dimension blthick(100,20,100000),tetime(100000), nnum(100)
...
C
      write(*,*)
      write(*,*) 'creates a bed thickness'
      write(*,*) 'time history for a given'
      write(*,*) 'set of nodes'
      write(*,*)
      ...
      write(*,*) 'node numbers'
      read(*,*) fnamen 
      write(*,*) 'enter input file prefix'
      read(*,*) fnamei
      write(*,*) 'enter number of bed layers'
      read(*,*) nblay
...
C
      ...
      if (ii .lt. 10) then
        open(10,file=trim(fnamei)//'_blt'//chint(ii)//'.dat',
     +  form='formatted',status='old')
      else
        open(10,file=trim(fnamei)//'_blt1'//chint(ii)//'.dat',
     +  form='formatted',status='old')
      end if
      ...
      do i = 1, 100000
        ...
        if(time .ge. tstart .and. time .le. tstop) then
          ...
          write(*,1650) time, ii
          do j = 1, nnode 
            read(10,*,end=1000, err=900) rdum
             do jj = 1, nnn
               if (j .eq. nnum(jj)) then
                 tetime(ntstep) = time
                 blthick(jj,ii,ntstep) = rdum
               end if
             end do
           end do
...
C
     ...
     +  form='formatted',status='unknown')
      ...
      do j = 1, ntstep
        do k = 2, nblay
          blthick(i,k,j) = blthick(i,k,j) + blthick(i,k-1,j)
        end do
        write(10,2000) nnum(i), tetime(j), 
     +  (blthick(i,k,j),k = 1, nblay)
      end do
      close(10)
      end do
...
C
 ...
 1650 format('processing time = ', f15.3, ' for file ', I8)
