...
C
     ...
     &        irec(40),eofile
      character filename*90,infile(maxfiles)*90
      logical here
     ...
     &        (vvel(j), j = 1, np), (dfct(j),j = 1, ne)
          ...
          if (tet.lt.tstart(n+1).and.svtimes(ntc).lt.tstart(n+1)) then
            if(istrt.eq.0) then
              tfst=tet
              istrt=1
            endif
            if(tet .eq. svtimes(ntc)) then
              write(55,*) tet,svtimes(ntc)
              write(12)
     &        tet, np, ndf, ne, ndfs,(iresav(k),k=1,ndfs),
     &        ((xvel(k,j),j = 1, iresav(k)),k=1,ndf),
     &        (wsel(j),j=1,iresav(3)),(imat(j),j=1,ne),(ndry(j),j=1,np),
  ...
  110 continue
      write(*,*) "Number of times saved: ",ntc-1
