...
C   AND TAKE HORIZONTAL "BOTTOM SLICES". THE DATA WILL
...
C
C   Last Modified by Barbara Donnell 09-21-1997
C 
C******************************************************************
C 
      PARAMETER (MAXNE =100000, MAXN =200000, MAXNDF = 6,
...
C 
     ...
     *                DELBED(MAXN),BSHR(MAXN),IZBV,NSNOD(MAXN)
     *                ,NETOT
C
...
C                      123456789-123456789-123456789-123456789-
      DATA BANSLC(1)/'CEWES-CHL PROGRAM SLICE_B version 2.1   '/
      DATA BANSLC(2)/'Last modified  09-21-1997               '/
C
...
C                      123456789-123456789-123456789-123456789-
      ...
      DATA BANRM2(3)/'RMA2-V VERSION 4.35         1-D AND 2-D '/
...
C GLB-NSF add the following initializations
   ...
   20 FORMAT(1X,A,/,1X,A,/,
     ...
     *   ' --> Echo of program activities on file=slice_bv21.txt',/)
      OPEN(99,FILE='slice_bv21.txt',FORM='FORMATTED',STATUS='UNKNOWN')
...
C
      IZBV = 0 
      WRITE(*,*) ' Enter a 0 for a slip boundary RMA10 simulation'
      WRITE(*,*) '         1 for a no slip boundary RMA10 simulation'
      READ (*,*) IZBV
      IF (IZBV .EQ. 0) THEN
        WRITE(*,*) ' You selected option 0.  This will generate a'
        WRITE(*,*) ' true bottom slice' 
      ELSE
        WRITE(*,*) ' You selected option 1. With respect to velocities,' 
        WRITE(*,*) ' This will slice the solution along the top of the'
        WRITE(*,*) ' bottom-most layer.  With respect to the other'
        WRITE(*,*) ' constituents, the slice will be taken along the'
        WRITE(*,*) ' bottom.' 
      END IF
      WRITE(*,*)  
C
...
C GLB-LDR enter request to show shallow elements as dry
      ISHOWDRY = 0
      WRITE (*,*) 'Want to show elements where the water surface'
      WRITE (*,*) 'elevation is less than the bed elevation as if they'
      WRITE (*,*) 'were dry elements? (DEF=NO)'
      READ (*,'(A)') REPLY
...
C GLB-LDR end addition
     ...
     *            '(DEF="S_VEL")'
      READ (*,'(A)') PREFIX1
      IF (PREFIX1(1:3) .EQ. '   ') PREFIX1 = 'SB_VEL'
C 
C 
...
C GLB-MCS change the following line to 'slice for a constituent'
     ...
     *              ' files (DEF="SB_SAL")'
        READ (*,'(A)') PREFIX2
        IF (PREFIX2(1:3) .EQ. '   ') PREFIX2 = 'SB_SAL'
      ENDIF
C
C 
C ... BpD wasn't totally sure what to put for ZSLICE question
      NSLICE = 1
      ZSLICE(1) = 0.0
      PRINT *,' -->  Will make one slice  (bottom) '
C 
      IREF = 0
...
C GLB remove NFIX1 (NX1DUM)
     *     ADUM, NXDUM, AO(J), NSNOD(J), J = 1, NPTOT), 
  ...
  210   FORMAT(' --> Number of Degrees of Freedom in the'
     *         ' solution file =', I2, ' Time read=',F12.5)
        IF (NDF2DO .GT. NDF) THEN
            PRINT *,' --> User ERROR .. cannot request to do more'
            PRINT *,'     degrees of freedom than are available'
            PRINT *,'     Hence NDF will be =',NDF
            NDF2DO = NDF
        ENDIF
...
C 
        DO 700 ISLICE = 1, NSLICE
          CALL BOTTOM (ISLICE, IDEBUG, SLIMIT, VLIMIT)
C GLB-LDR add this to make elements where wsel < ao at
C GLB-LDR every node appear dry
          IF (ISHOWDRY .EQ. 1) THEN
            DO K = 1, NE
              IDELE = 1
              IMAT(K)=IABS(IMAT(K))
              DO J = 1, 20
                IF (NOP(K,J) .NE. 0) THEN
                  IF (WSEL(NOP(K,J)) .GT. AO(NOP(K,J))) THEN
                    IDELE = 0
                  END IF
                  IF (J .GE. 9) IDELE = 0
                END IF
...
C
  850 IF (ABS(TIME2GET(1)-99999.).LT. .01) THEN
          TIME2GET(1) = TIME
          WRITE(*,*) ' Found the last record ... time (hours)=',TIME
...
C********************************************************************
C 
      SUBROUTINE bottom (ISLICE, IDEBUG, SLIMIT, VLIMIT)
      SAVE
C 
C  THIS ROUTINE WILL TAKE THE DATA FROM AN RMA10 RUN & RETURN
C  A HORIZONTAL SLICE (AT AN DEPTH = bottom)
C 
C    INPUTS:
C 
C     ZSLICE(ISLICE) = THE DEPTH OF THE HORIZONAL SLICE
...
C
     ...
     *                NDRY(MAXN), IMAT(MAXNE)
      ...
      DIMENSION NOPSUR(8)
      DIMENSION ICNFLG(MAXN)
C ... REAL star 8 needed for HEC version of RMA10 solution files
C      REAL*8  AO        
C 
C*****************************************************************
C 
      IF (ITRACE.GE.1) PRINT *,' --> routine bottom'

      NPS      = NP
      NOFIND   = 0
C 
C    FIND MIDSIDE NODES (i.e. ICNFLG = 0)
      DO J = 1, NETOT
        DO I = 1, 20 
          N = NOP(J,I)
          IF (ABS(NSNOD(N)) .EQ. N) THEN
            IF (N .NE. 0) THEN
              ICNFLG(N) = 0
              IF (MOD(I,2) .NE. 0) ICNFLG(N) = 1
            END IF
          END IF
        END DO
      END DO
...
C 
          DO 250 NDFF = 1, NDF
             NSUBT = 0
             IF (NDFF .LE. 2) THEN 
               IF (IZBV .EQ. 1 .AND. ICNFLG(I) .EQ. 0) NSUBT = 1
               IF (IZBV .EQ. 1 .AND. ICNFLG(I) .EQ. 1) NSUBT = 2
             END IF
             NDEEP2 = NDEEP-NSUBT
             IF (NDFF .NE. 3) THEN
               VEL(NDFF,I) = XVEL(NDFF,NDEEP2)  
             END IF 
  250     CONTINUE
C
       END IF 
