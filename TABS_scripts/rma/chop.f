...
c
      ...
      DS = 0.
C
C GLB do some work to  
C     avoid complications with slicing through zero imats
C
      ELEV = -1000.0
      DO J = 1,NS
        NN = NODES(J)
        ELEV = AMAX1(ELEV,CORD(NN,3)) 
      END DO
      DO J = 1,NS
        NN = NODES(J)
        CORD(NN,3) = ELEV
      ...
      DO J = 2, NS-1, 2
        NN = NODES(J)
        DO K = 1,2
          RMIDV = (CORD(NODES(J-1),K)+CORD(NODES(J+1),K))/2.
          RDIFF = ABS(CORD(NN,K) - RMIDV) 
          IF (RDIFF .GT. 1.E-3) THEN 
            CORD(NN,K) = RMIDV
          END IF
        END DO
      END DO
...
C GLB add the following clause
      ...
      NM = NODEC(NOPN(I,J-1))
      NP = NODEC(NOPN(I,JP))
      if (nm .eq. 0 .or. np .eq. 0) goto 310
