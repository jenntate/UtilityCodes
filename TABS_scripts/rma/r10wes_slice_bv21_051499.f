...
C ...    time window                    
   ...
   94 FORMAT(I8,' ndf2do')
      ...
      READ (*,'(A)') REPLY
      IF (REPLY .EQ. 'n' .OR. REPLY .EQ. 'N') GO TO 100
C
...
C
  100 WRITE (*,*) ' Enter the base elevation (DEF = 100 FEET)'
      ELEV0 = 100.0
      CALL DCODE(ELEV0, ELEV0)
C 
...
C 
      IREF = 0
      WRITE(*,*) ' Want to reference slices to a fixed elevation?'
      READ (*,'(A)') REPLY
      IF (REPLY .EQ. 'Y' .OR . REPLY .EQ. 'y' ) THEN
        WRITE(*,*)  ' Enter the reference elev below which to slice' 
        READ (*,*)  REFEL
        IREF = 1
      ENDIF
