...
C
  65  PRINT *,'Enter the RMA10 Sed binary solution file to analyze:'
      READ(*,'(A)')  FNAME
         INQUIRE (FILE=FNAME, EXIST=AROUND)
         IF (.NOT.AROUND) THEN
             PRINT *,' --> File not found ... try again'
         ...
         ENDIF
      WRITE(44,'('' RMA10 : '',A)') FNAME
      OPEN(UNIT=2,FILE=FNAME,FORM='UNFORMATTED',STATUS='OLD')
C
...
C ...    Loop through the RMA2 binary solution
         PRINT*, 'at the call to read the solutions files'
         CALL R10READ
...
C   
C -------
        PRINT*, 'Inside the R10READ Subroutine'
        LUIN2 = 1
        NDF = 6
        NDFP1 = NDF + 1
C ...   RMA10-WES binary solution read

C ...   JNT LOOP THROUGH HYDRO TO SKIP 3 MONTHS OF SPINUP
        PRINT*, 'At the first READ of the Hydro file'
  ...
  500   READ (LUIN2,END=2000) TET, NP1, NDF, NE1,
     ...
     &       (RDUM, J = 1, NP1), (RDUM,
     &       J = 1, NE1)
        PRINT*, 'Solution in English...', VEL(1,1)

        DO K=1,3
          DO J=1,IRESAV(K)
            VEL(K,J)=0.3048*VEL(K,J)
          ENDDO
        ENDDO

        PRINT*, 'Converted to metric...', VEL(1,1)
        PRINT*, 'Read the hydro solutions, Time = ', TET
C        PRINT*, 'Binary nodes = ', NP1
C        PRINT*, 'Binary elements = ', NE1
C        PRINT*, 'Geometry nodes = ', NPTOT
C        PRINT*, 'Geometry elements = ', NETOT

       IF (TET .LT. 2208.0) THEN
        GOTO 500
       ENDIF
       TETH=TET
      IF (NP1 .NE. NPTOT .OR. NE1 .NE. NETOT) THEN
        ...
        STOP
      END IF
C ...  JNT added to read sediment solution file
        LUIN2 = 2
  700   READ (LUIN2,END=2000) TET, NP1, NDF, NE1,
     &       NDFS,(IRESAV(K),K=1,NDFS),
     &       ((VEL(K,J),J = 1, IRESAV(K)),K=1,NDF),
     &       (VTEMP,J = 1, IRESAV(3)),
     &       (ITEMP, J = 1, NE1), (ITEMP, J = 1, NP1),
     &       (RDUM,J=1,IRESAV(NDFP1)),(RDUM,J=1,IRESAV(NDFS)),
     &       (RDUM, J = 1, NP1), (RDUM,
     &       J = 1, NE1)
      IF (NP1 .NE. NPTOT .OR. NE1 .NE. NETOT) THEN
      ...
      END IF
      IF (TET .NE. TETH) THEN
        Write(*,*)'Error: timesteps do not match'
        Write(*,'(''Hydro:  '',F12.5)') TETH
        Write(*,'(''Sed:  '',F12.5)') TET
        STOP
      ENDIF
