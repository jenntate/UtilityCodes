...
*     GFGEN & RMA2 (VERSION 4.25 or higher) & RMA4 & RMA10    
*                             LAST MODIFICATION DATE: 01-27-1992
*     DONNELL RE-MODIFIED TO ACCEPT VERSION 4.27    : 11-08-1993
*     BPD_G.Brown MODIFIED TO ACCEPT RMA10 REVISIONS: 10-09-1998
*     DONNELL fixed R4INT & R4MERG bin sol write err: 04-29-1999
****************************************************************
*
...
C
      PARAMETER(MME=45000,MMN=90000,NTS=4000)
C
      INTEGER TRMLEN
      LOGICAL HERE
      CHARACTER*80   DMSBAN
...
C
     ...
     *               NEG,NPG,NE2,NP2
C
      COMMON /BLKR4/ TIMER4, NQAL, NPR4, NER4, IMAT4(MME), CON(MMN,6) 
C
C ... GLB-R10 11-8-93 change VELR10 dimension from 4 to 6
...
C ...                                           GLB-R10 11-8-93     
      COMMON /BLKR10_SED/ IRESAV(8),DELBED(MMN),BSHR(MMN),DFCT(MME)
C
      HERE = .FALSE.
C
      DO 1 I= 1, MMN
...
C
      ...
      PRINT *,' ***    Last Modified:  April 29, 1999    ***'     
      PRINT *,' ********************************************'  
      PRINT 8, MME, MMN
    8 FORMAT(/,
     *       ' Dimensioned for ',I6,' Elements, and ',I6,' Nodes',/,
     *       ' Program MERGAVEv30 was last modified 04-29-1999')
      PRINT *,' ********************************************'
...
C ---
   50 ISKIP = 1
C
C     IF(NE2.NE.NEG .OR. NP2.NE.NPG) WRITE(6,200) NEG,NPG,NE2,NP2
...
C -----------------------------------------------------------------
  ...
  670    PRINT *,' Enter RMA4 averaged file name to be created'
         ...
         ENDIF
C
  700    PRINT *,' Enter name of RMA4 file to read from' 
         READ(5,40,END=470) IFILEM
         LASTC = TRMLEN( IFILEM )
         INQUIRE (FILE=IFILEM, EXIST=HERE)
         NSAL = 70
...
C
      ...
      IDEBUG = 4
      PRINT *,' DO you want to turn full prints on? (Enter 1 if yes)'
      READ(5,*,END=10) ITEST
      IDEBUG = ITEST
   10 FLAG = 'N'
...
C
      IF (NE2.NE.NEG .OR. NP2.NE.NPG) 
     *                     WRITE(6,6020) NPG, NEG, NP2, NE2
 6020 FORMAT(/,' WARNING inconsistant node/elem... RMA4 vs GFGEN',/,
     *       ' GFGEN had node/elem =', 2I8,/,
     *       ' RMA2  had node/elem =', 2I8,/)
      RETURN
      END
C ------
      SUBROUTINE R4MERG
C                                           Last Mod 4-29-1999
...
C ...        This must be a character banner style binary file
             ...
             IF( I.EQ.1 ) THEN
                 PRINT *,' --> Write CHARACTER Banners to unit=80'
...
C-
             IF( I.EQ.1 ) THEN  
                 PRINT *,' --> Write INTEGER/CHAR Banners to unit=70'
     ...
     *                         (IREC(J),J=1,IWRT2),(FREC(J),J=1, IWRT3)
         ...
         WRITE(*,100) MFLG, ISWREC, NPR4, NER4, 
     *                    IWRT2, IWRT3, IWRT4
  100    FORMAT(/, ' Just processed RMA-4 Banners AOK. ',/,
     *        ' --> Model Flag (variable MFLG)=     ',I8,/,
     *        ' --> Your RMA4 has a version # of:   ',I8,/,
     *        ' --> Your RMA4 total nodes/elements: ',2I8,/,
     *        ' --> Integer, Real, Title Records:   ',3I5,/)         
...
C 
 ...
 3060 FORMAT('  ==== MERGED RMA4 file named -->',A,/,
     *       '  ==== Closed with',I6,' time steps',/)
      IF (NER4.NE.NEG .OR. NPR4.NE.NPG) 
     *                     WRITE(6,3080) NPG, NEG, NPR4, NER4
 3080 FORMAT(/,
     *       ' WARNING inconsistant node/elem... RMA4 vs GFGEN',/,
     *       ' GFGEN had node/elem =', 2I8,/,
     *       ' RMA4  had node/elem =', 2I8,/)
      RETURN
      END
C --------
      SUBROUTINE R4INT 
...
C
   30 IRECN = 0 
      ...
      PRINT *,' INSIDE R4INT, rewind nsal=70 to read rma-4 binary'
C ... determine type of binary file and read 
      READ (NSAL,END=50,ERR=50) ITEST4
      PRINT *,' --> RMA4 binary 1st integer ITEST =',ITEST4
      REWIND (NSAL)
      IF (ITEST4.GT.200) THEN
C ...     This must be a character banner style binary file
          ...
          IRECN = 2
          IF (ISWREC.GT.425) READ (NSAL) TITLER4
          IRECN = 3
          IF( I.EQ.1 ) THEN
              PRINT *,' --> WRITE CHARACTER BANNERS TO UNIT=50'
...
C ...     Integer style character values
     ...
     *                 (IREC(J),J=1,IWRT2),(FREC(J),J=1, IWRT3)
          ...
          CALL CONVRT ( TITLER4, 1, IPACKT, 77, 2)
C-          
          PRINT *,' --> Write INTEGER/CHAR Banners to unit=80'
     ...
     *                    (IREC(J),J=1,IWRT2),(FREC(J),J=1, IWRT3)
      ...
      ENDIF
      WRITE(*,40) MFLG, ISWREC, NPR4, NER4, 
     *                    IWRT2, IWRT3, IWRT4
   40 FORMAT( ' Just processed RMA-4 Banners AOK. ',/,
     *        ' --> Model Flag (variable MFLG)=     ',I8,/,
     *        ' --> Your RMA4 has a version # of:   ',I8,/,
     *        ' --> Your RMA4 total nodes/elements: ',2I8,/,
     *        ' --> Integer, Real, Title Records:   ',3I5,/)
...
C-
  250 RECN = 0.
      DO 3000 I = 1, NTS
         READ(NSAL,END=4000,ERR=4000)  TIMER4, 
     *             NQAL, NPR4, ((CON(J,K),J=1,NPR4),K=1,NQAL),
     *             NER4, (IMAT4(II), II=1,NER4)
C
...
C ***         FIND MINIMUM AND MAXIMUM FOR EACH NODE OVER THE CYCLE
 ...
 2000    CONTINUE
         GO TO 3000
 2500    WRITE(3,2510) TIMER4, NQAL, NPR4, NER4
 2510    FORMAT(' Skip RMA4 Time=',F15.6,
     *          '  NQAL=',I3,' Nodes=',I8, 'Elem=',I7)
 3000 CONTINUE
C
      IF( RECN.LE.1.0) THEN
            PRINT 3050,   RECN, T2START, T2STOP
...
C
        PRINT *,' Write RMA4 Binary integrated data on unit=80'
        TIMER4 = 0.0       ! temporary SMS fix 
C       TIMER4 = -99.      ! the proper time should be

        WRITE(80) TIMER4, 
     *            ITOTCON, NPR4, ((CAVE(J,K),J=1,NPR4),K=1,ITOTCON),
     *            NER4, (IMAT4(II), II=1,NER4)     
C
      WRITE(6,6010) T2START, T2STOP, TIMER4, NPR4, NER4, ITOTCON
      WRITE(3,6010) T2START, T2STOP, TIMER4, NPR4, NER4, ITOTCON
 6010 FORMAT(/,' COMPLETE-',/,
     * ' UNIT 80 CONTAINS 1 TIME STEP OF AVERAGED CONCENTRATIONS',/,
     * ' From Hrs',F15.7,' to ',F15.7,/,
     * ' Time on RMA4 binary solution is flagged as hr  =',F8.3,/,
     * ' Total nodes written on RMA4 binary solution    =',I8,/,
     * ' Total elements written on RMA4 binary solution =',I8,/,     
     * ' Total # of constituents written on RMA4 bin sol=',I3,/)
     
      IF (NER4.NE.NEG .OR. NPR4.NE.NPG) 
     *                     WRITE(6,6020) NPG, NEG, NPR4, NER4
 6020 FORMAT(/,
     *       ' WARNING inconsistant node/elem... RMA4 vs GFGEN',/,
     *       ' GFGEN had node/elem =', 2I8,/,
     *       ' RMA4  had node/elem =', 2I8,/)
...
C ...                                           GLB-R10 11-8-93     
      COMMON /BLKR10_SED/ IRESAV(8),DELBED(MMN),BSHR(MMN),DFCT(MME)
C
C ... GLB-R10 11-8-93 change dimension 4 to 6 in the following arrays
...
C -------
      ...
      CHARACTER *(*) STRING
      ILAST = LEN(STRING)
      DO 10 I = ILAST, 1, -1           ! modified 12-4-98
