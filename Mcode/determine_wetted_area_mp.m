function output=determine_wetted_area_mp(basename,grid_filename)
%
%
...
fprintf(1,'Reading da file header\n');
check=load_da_stepnew([basename '.da'],0);
fprintf(1,'Finished reading da file header\n');
...
for i=1:check.nt
   ...
   if(mod(i,50)==0)
     fprintf(1,'Reading time step number %i\n',i);
   end 
   ...
   v_mag=sqrt(data.v.*data.v+data.u.*data.u);
   data.dep(v_mag==0.0)=-0.1;
...
save([basename '_wetted_area'],'output')
