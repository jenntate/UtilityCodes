function vel_analysis(vel_field_data_file,plot_title,y_label, ...
...
%  tempbbb=wse_field_data_file{i,1}
  ...
  field.valuex=field.valuex*multi;
  ...
  data_x(i)=perform_model_versus_field_analysis(field.time(start_field:stop_field),  ...
        field.valuex(start_field:stop_field), [name '_(X_Component)'], ...
        ['X Component of ' y_label],plot_title, ...
...
for i=1:length(vel_field_data_file(:,1))  
  if(~isempty(data_x(i).RMSE))
    ...
    fprintf(out,'X COMPONENT OF VELOCITY INFORMATION\n');
