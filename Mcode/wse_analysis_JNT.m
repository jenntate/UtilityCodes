...
%  fprintf(1,'\nFinding nearest node to comparison location %i\n',i)
  ...
  outf=fopen(['field_datafile_' int2str(k) '.dat'],'w','n'); 
  outm=fopen(['model_datafile_' int2str(k) '.dat'],'w','n'); 
...
%  tempbbb=wse_field_data_file{i,1}
  ...
  data(i)=perform_model_versus_field_analysis_JNT(...
        field.time(start_field:stop_field),  ...
  ...
  for t=start_field:stop_field
    fprintf(outf,'%f %f \n', field.time(t), field.value(t));
  end

  for t=start_model:stop_model
    fprintf(outm,'%f %f \n', model.time(t), model.wse(t));
  end
...
for i=1:length(wse_field_data_file(:,1))  
  if(~isempty(data(i).RMSE))
    ...
    fprintf(out,'Willmott Coefficient            = %5.2f\n', ...
        data(i).Willmott);      
    fprintf(out,'Pearson Coefficient            = %5.2f\n', ...
        data(i).Pearson);      
