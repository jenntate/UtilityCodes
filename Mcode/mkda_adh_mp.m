function mkda_adh_mp(basename,t0,attributes)
%
...
%    fprintf(1,'Time = %f\n',dep.time(count))
  ...
  if(slt_check > 0)
    for i=1:slt_check
      ...
      if(isequal(compare,'ENDDS'))
        ...
        hold=textscan(fid_slt(i),'%f %f %f',slt.np);
        slt.silt(i,:)=hold{1};
        slt.rouse(i,:)=hold{2};
        slt.bdma(i,:)=hold{3};
      end
  ...
  clear zero_out;
  zero_out(1)=0;
  loc=find(ovl.velocity(1,:)==0);
  count111=1;
  for qq=1:length(loc)
     if(ovl.velocity(2,loc(qq))==0)
       zero_out(count111)=loc(qq);
       count111=count111+1;
     end
  end
%  find(zero_out==0)
%  length(zero_out)
    ...
    if(over == 0)
      if(dep_check > 0)
        if(zero_out(1)~=0)
          for qq=1:length(zero_out)
             dep.depth(zero_out(qq))=-1.0;
          end
        end
        time_check=1;
        fprintf(1,'Time = %f\n',dep.time(count));
      ...
      if(vel_check > 0)
        ...
        if(zero_out(1)~=0)   
          for qq=1:length(zero_out)
             ovl.velocity(1,zero_out(qq))=0.0;
             ovl.velocity(2,zero_out(qq))=0.0;
          end
        end
      ...
      if(snd_check > 0)
        if(time_check == 0)
           fprintf(1,'Time = %f\n',snd.time(1,count));
           fwrite(fid,snd.time(1,count)*tconv+t0,'float64');
           time_check=1;
        end   
        for qq=1:length(zero_out)
           snd.sand(:,zero_out(qq))=-1.0;
           snd.rouse(:,zero_out(qq))=-1.0;
           snd.bdma(:,zero_out(qq))=-1.0;
        end
      ...
      if(slt_check > 0)
        if(time_check == 0)
           fprintf(1,'Time = %f\n',slt.time(1,count));
           fwrite(fid,slt.time(1,count)*tconv+t0,'float64');
           time_check=1;
        end   
        for qq=1:length(zero_out)
           slt.silt(:,zero_out(qq))=-1.0;
           slt.rouse(:,zero_out(qq))=-1.0;
           slt.bdma(:,zero_out(qq))=-1.0;
        end
      ...
      if(dpl_check > 0)
        ...
        for qq=1:length(zero_out)
           dpl.displacement(zero_out(qq))=-1.0;
        end
      ...
      if(bed_check > 0)    
        ...
        for qq=1:length(zero_out)
           bed.value(1,zero_out(qq))=0.0;
           bed.value(2,zero_out(qq))=0.0;
        end

        fwrite(fid,bed.value(1,:),'float32');
        fwrite(fid,bed.value(2,:),'float32');
      end
      ...
      if(cbp_check > 0)
        ...
        for qq=1:length(zero_out)
           cbp.den(:,zero_out(qq))=-1.0;
           cbp.ces(:,zero_out(qq))=-1.0;
           cbp.erc(:,zero_out(qq))=-1.0;
           cbp.ere(:,zero_out(qq))=-1.0;
        end
