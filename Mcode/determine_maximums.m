function determine_maximums(basename,attributes,start_time,stop_time)
%
%
...
%     datestr(data.time)
     ...
     if(vel_check > 0)
       for j=1:check.np
          vel_mag(j)=(data.u(j)*data.u(j)+data.v(j)*data.v(j))^0.5;
          velx(j)=data.u(j);
          vely(j)=data.v(j);
       end
