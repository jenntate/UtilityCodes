function plot_data(basename,time,value,yname,attribute,point_number, ...
    units,fid,start_time,stop_time,len)
%
...
%   Input:
%
%       basename - basename for the model simulations (.da files)
%
...
%       simulation
%
%       yname - name to use for the y-axis label
%
%       attribute - the attribute being plotted
%
%       point_number - the point number which is being plotted and analyzed
%
...
for i=1:length(basename)
   min_x1(i)=min(time(i,1:len(i))); 
   max_x1(i)=max(time(i,1:len(i)));
end
...
for i=1:length(basename)
   ...
   loc1=find(time(i,1:len(i))>=min_x,1,'first');
   loc2=find(time(i,1:len(i))<=max_x,1,'last');
   
   % calculates the minimim, maximum, mean, and median values
   
   min_y1(i)=min(value(i,loc1:loc2));
   max_y1(i)=max(value(i,loc1:loc2));
   mean_y1(i)=mean(value(i,loc1:loc2));
   median_y1(i)=median(value(i,loc1:loc2));
   
   % prints the values to the output file
   
   fprintf(fid,'Data for file %s from time %s to %s\n\n',basename{i}, ...
       datestr(min_x),datestr(max_x));
   fprintf(fid,'Minimum %s Value for Point %i is %f\n',yname,point_number,...
       min_y1(i));
   fprintf(fid,'Maximum %s Value for Point %i is %f\n',yname,point_number,...
       max_y1(i));   
   fprintf(fid,'Average %s Value for Point %i is %f\n',yname,point_number,...
       mean_y1(i));  
   fprintf(fid,'Median  %s Value for Point %i is %f\n\n',yname,point_number,...
       median_y1(i));     
...
for i=1:length(basename)
    ...
    plot(time(i,1:len(i)),value(i,1:len(i)),'Parent',axes1,'LineWidth',0.5,'DisplayName',name);
end
...
ylabel([yname ', ' units],'FontSize',16,...
    'FontName','Times New Roman');

%% Create the plot title

title([yname ' Point Comparisons'],'FontSize',16,...
    'FontName','Times New Roman');
...
%% saves the png and fig files with the point number and attribute in the filename

set(gcf,'renderer','zbuffer');
opengl neverselect
set(gcf,'Backingstore','off','doublebuffer','on');
saveas(gcf,['Point_Number_' num2str(point_number) '_' attribute],'png');
saveas(gcf,['Point_Number_' num2str(point_number) '_' attribute],'fig'); 
close(gcf);   