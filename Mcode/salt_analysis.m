function salt_analysis(salt_field_data_file,plot_title,y_label, ...
...
nearest=zeros(length(salt_field_data_file(:,1)),1);
temp=cell(length(basename),1);

for i=1:length(salt_field_data_file(:,1))
...
%  fprintf(1,'\nFinding nearest node to comparison location %i\n',i)
  ...
  k=nearestNeighbor(dt, ...
      [salt_field_data_file{i,2} salt_field_data_file{i,3}]);
  ...
  if(length(basename) > 1)
     for h=2:length(basename)
        ...
        model.sal(length(model.sal)+1:length(model.sal)+1+length(temp{h}.sal)-loc) = ...
            temp{h}.sal(loc:length(temp{h}.sal));  
        clear temp
     end
  end  
  ...
  start=find(salt_field_data_file{i,1}=='\',1,'last')+1;
  start1=find(salt_field_data_file{i,1}=='/',1,'last')+1;
  if(isempty(start) && isempty(start1))
      start=1;
  elseif(isempty(start))
      start=start1;
  end
  stop=find(salt_field_data_file{i,1}=='.',1,'last')-1;
  name=salt_field_data_file{i,1}(start:stop);  
  clear start stop  
  
  model.sal=model.sal*salt_field_data_file{i,5};
  field=read_data(salt_field_data_file{i,4},salt_field_data_file{i,1});

  multi=salt_field_data_file{i,6};
  ...
  data(i)=perform_model_versus_field_analysis(field.time(start_field:stop_field),  ...
        ...
        model.sal(start_model:stop_model), 3,shift,harm);    

end

for i=1:length(salt_field_data_file(:,1))  
  if(~isempty(data(i).RMSE))
    fprintf(out,'%s\n\n',salt_field_data_file{i,1});
