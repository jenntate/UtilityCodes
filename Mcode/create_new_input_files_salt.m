function create_new_input_files_salt(old_base_file_name,new_base_file_name)
%
%
%
...
%               end
             ...
             %         wse(new_match_to_old(y)));
               ...
               for y=1:length(new_grid.x)
                 if(new_match_to_old(y) ~= 0)
                    if(data{1}(new_match_to_old(y)) > 30.0)
                        data{1}(new_match_to_old(y))=30.0;
                    end
                    if(data{1}(new_match_to_old(y)) < 0.0)
                        data{1}(new_match_to_old(y))=0.0;
                    end                    
                    fprintf(fid_out_hot,'%15.8e\n',...
                      data{1}(new_match_to_old(y)));
                 else
