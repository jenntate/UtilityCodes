function analysis_output_new(screen_filename,adh_root_file)
%
...
%
%   The code writes time step, nonlinear iteration, and linear iteration
%   information to the run_information.out file.
%
%	INPUT VARIABLES
%
...
%
%	analysis_output_new('adh.out','Mobile')
%
%   Finished 3/12/2010
%
...
%   end
   ...
   if(~isempty(checker))
     ...
     output=textscan(hold,'%*s %f %f %*1s %f %f %f %f %f %f %f %f %f %f %f %f %f %f %s',1);
...
%       max_resid_norm(count)
       ...
       count=count+1;
     elseif(~isempty(output{17}))
       if(NTL_tol > output{6})
         bad_node_failure(count_bad)=output{12};
%         bad_node_failure(count_bad)
         count_bad=count_bad+1;
       end
       if(ITL_tol > output{11})
...
fid_info=fopen('run_information.out','w');

createfigure_bar_dt(dt(1:count-1),fid_info)
...
createfigure_bar_non_it(non_it(1:count-1),fid_info)
...
createfigure_bar_lin_it(lin_it(1:count-1),fid_info)
...
for i=1:count-1
   ...
   if(inc_max_norm_node(i) == -2)
     k=nearestNeighbor(dt_interp,[inc_max_norm_node_x in_max_norm_node_y]); 
     node=k;
   else
...
fprintf(fid1,'Total Number of Converged Time Steps = %i\n',length(sorted_bad));
fprintf(fid1,'x-coor   y-coor  number failures   node number\n');
...
fprintf(fid2,'x-coor   y-coor  number failures   node number\n');
for i=1:count-1
